...
const getFormattedTemp = (temp) => {
  const coloredTemp = ansiSymbols.getColoredTemp(Math.round(temp * 1.8 + 32))
...
const getFormattedWind = (direction, speed) => {
  ...
  const coloredSpeed = ansiSymbols.getColoredWindSpeed(roundedSpeed)

  return `${arrow} ${coloredSpeed} mph`
}
